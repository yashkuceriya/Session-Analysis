...
export const FloatingSelfView = forwardRef<HTMLDivElement, FloatingSelfViewProps>(
  function FloatingSelfView({ videoRef, name, isMuted = false, eyeContactScore }, ref) {
    ...
    const getCornerCoordinates = useCallback((corner: CornerPosition): { x: number; y: number } => {
      ...
      switch (corner) {
        ...
        case 'bottom-left':
          return { x: margin, y: window.innerHeight - h - margin };
        case 'top-right':
    ...
    return (
      <div
        ref={containerRef}
        className={`fixed z-30 rounded-2xl overflow-hidden bg-gray-900 ring-2 ring-gray-700 shadow-2xl transition-all ${
      ...
      >
        ...
        <div className="absolute bottom-2 left-2 flex items-center gap-1">
          <div className="bg-black/60 backdrop-blur-sm px-2 py-0.5 rounded text-xs flex items-center gap-1">
        ...
        <div className="absolute top-2 right-2">
          <div className={`w-2 h-2 rounded-full ${getAttentionDot()} shadow-lg transition-colors duration-500`} />
        </div>
        ...
        {!isDragging && (
          <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity bg-black/20 backdrop-blur-sm">
            <span className="text-white text-xs font-medium">Drag to move • Double-click to resize</span>
          </div>
        )}
      </div>
    );
